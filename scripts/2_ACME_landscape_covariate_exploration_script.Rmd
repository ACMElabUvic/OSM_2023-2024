...
### Alternative grouping with well_aband

> Marissa added this section to separate the well_aband column from the wells column for the combined ABMI and OSM report. This may be something we want to consider doing in the future? Not sure

```{r}
covariates_grouped_abmi <- covariates_fixed %>% 
...
# see what's left
names(covariates_grouped_abmi)

```

Check this data more closely
```{r}


# check the structure of new data
str(covariates_grouped_abmi)

# check summary of new data
summary(covariates_grouped_abmi)
```

Save data

```{r save covariates abmi}

write_csv(covariates_grouped_abmi,
          'data/processed/OSM_ABMI_covariates_grouped_2021_2022_2023.csv')
```

And remove from environment so we don't accidentally use for ACME OSM stuff

```{r}
rm(covariates_grouped_abmi)
```
