...
```{r}
write_csv(osm_coords_deploy, "./data/processed/OSM_site_deployment_coordinates_2021_2022_2023.csv")
...
covariate_data <-    
  ...
  file.path('data/raw/',
            ...
            c('OSM_LU09_LU14_LU16_LU22_HFI_2023_20250209.csv',
              'OSM_LU09_LU14_LU16_LU22_VEG_2023_20250209.csv')) %>%
...
There are 199 for both data sets and I don't see any glaring issues but we can use a function in R to check that these match perfectly.
...
# we need to check that they also match the osm_2023_Det
...
# [1] "LU14_57"
# This site was unretrievable due to flooding, so this makes sense! We will retain the covariates from this site. In case this camera is ever retrieved, we will not need to re-extract covariates. 
```
...
write_csv(covariates_all,
          'data/processed/OSM_covariates_2023.csv')
...
covariates_all_wide <- covariates_all %>% 
  ...
  select(site, buff_dist, harvest_area:lc_class230) %>% 
...
```{r save wide format covariates, eval = FALSE}

# save wide format data 
write_csv(covariates_all_wide,
          'data/processed/OSM_covariates_wide_2023.csv')
```
