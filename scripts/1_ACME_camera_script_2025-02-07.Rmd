...
monthly_presence <- detections %>%
  ...
  distinct(array, site, species, month) %>% # keep one copy of detections for each week
...
Second, let's merge our monthly presence into the list of *all* operating months. Where NAs are introduced into the *presence* column, there was no detection of the given species in that month. We will replace these NAs with "0" to denote 'true' absence. 
...
monthly_presenceabsence <- deploy_active_months %>%
  ...
  left_join(monthly_presence, by = c("array", "site", "month", "species")) %>%
